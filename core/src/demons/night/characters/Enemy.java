...
public class Enemy implements Disposable {
    ...
    public Enemy() {
            ...
            if(ResourceManager.dificultad.equals("facil")){
                WALKING_SPEED = 0.50f;
            }
            ...
            rightAnimation = new Animation<TextureRegion>(0.15f, atlas.findRegions("skeleto_right"));

            // Crea la animación para correr hacia la izquierda
            leftAnimation = new Animation<TextureRegion>(0.15f, atlas.findRegions("skeleto_left"));
    ...
    private boolean comprobarCaida(float x, float y){
        boolean caida = false;
        ...
        float x_destino=0;
        if(faceLeft){
            x_destino = position.x - 8f;
        }else{
            x_destino = position.x + 16f;
        }
